...
describe("write-cli-startup-metadata", () => {
  ...
  it("hard-kills synchronous root help renderers after their timeout", async () => {
    const tempRoot = createTempDir("openclaw-startup-metadata-root-help-");
    writeFixtureFile(tempRoot, "root-help-fixture.js", "export function outputRootHelp() {}\n");
    ...
    spawnSyncMock.mockReturnValueOnce(successfulRender).mockReturnValueOnce(successfulRender);

    await expect(renderBundledRootHelpText(tempRoot)).resolves.toBe("Usage: openclaw\n");
    expect(__testing.renderSourceRootHelpText()).toBe("Usage: openclaw\n");

    expect(spawnSyncMock).toHaveBeenCalledTimes(2);
    for (const [, , options] of spawnSyncMock.mock.calls) {
      expect(options).toMatchObject({ killSignal: "SIGKILL", timeout: 120_000 });
    }
  });
